...
import { Plus, TrendingUp, TrendingDown, Target, Activity, BarChart3, ArrowUpRight, ArrowDownRight, Minus, Sparkles, LineChart } from 'lucide-react';
...
interface DashboardProps { trades: Trade[]; onAddTrade: () => void; tradesLoading?: boolean; }
...
export default function Dashboard({ trades, onAddTrade, tradesLoading }: DashboardProps) {
  ...
  const changePeriod = (p: Period) => {
    setPeriod(p);
    try { localStorage.setItem(PERIOD_STORAGE_KEY, p); } catch {}
  };
  ...
  const quant = useMemo(() => computeQuantStats(filtered, startingBalance), [filtered, startingBalance]);
  const recentTrades = useMemo(() => [...filtered].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 8), [filtered]);
    ...
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto">
      ...
      {trades.length === 0 ? (
        ...
        <div className="glass rounded-3xl p-8 md:p-14 text-center card-premium animate-fade-in-up stagger-1 relative overflow-hidden">
          ...
          <div className="max-w-sm mx-auto mt-8 text-left opacity-50 pointer-events-none select-none" aria-hidden="true">
            <div className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold mb-2">{t('empty.example')}</div>
            <div className="glass rounded-xl px-4 py-3 flex items-center gap-3 border-dashed">
              <div className="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center shrink-0"><ArrowUpRight className="w-4 h-4 text-emerald-400" /></div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-white">NQ</span>
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-400">L</span>
                  <span className="text-[10px] text-slate-600">Silver Bullet</span>
                </div>
                <div className="text-[10px] text-slate-600">10:03 · 2R · $150 {t('dashboard.riskSuffix')}</div>
              </div>
      ...
      <div className="relative glass rounded-3xl p-4 md:p-6 card-premium animate-fade-in-up stagger-1 overflow-hidden mb-4 md:mb-6">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
          <div>
            ...
            <div className="flex items-baseline gap-3 flex-wrap">
              <span className={cn('text-3xl md:text-4xl font-bold tabular-nums tracking-tight', gain ? 'text-emerald-400' : 'text-red-400')}
                style={{ textShadow: gain ? '0 0 24px rgba(16,185,129,0.25)' : '0 0 24px rgba(239,68,68,0.25)' }}>
                {formatPnl(stats.totalPnl)}
              </span>
              {periodPct !== null && (
                <span className={cn('text-xs md:text-sm font-bold px-2 py-0.5 rounded-lg tabular-nums',
                  gain ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400')}>
                  {periodPct >= 0 ? '+' : ''}{(periodPct * 100).toFixed(2)}%
                </span>
              )}
              <span className="text-[11px] text-slate-500">{stats.totalTrades} {t('common.trades')}</span>
            </div>
          </div>
          {/* Period selector */}
          <div className="flex items-center gap-1 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
            {PERIODS.map(p => (
              <button key={p} onClick={() => changePeriod(p)}
                className={cn('px-2.5 md:px-3.5 py-1.5 rounded-lg text-[11px] md:text-xs font-bold uppercase transition-all',
                  period === p ? 'bg-cyan-500/15 text-cyan-300 shadow-[0_0_12px_rgba(34,211,238,0.15)]' : 'text-slate-500 hover:text-slate-300')}>
                {p === '7d' ? '7D' : p === '30d' ? '30D' : p === 'ytd' ? 'YTD' : t('common.all')}
              </button>
            ))}
          </div>
        </div>
        ...
        ) : (<div className="h-56 md:h-80 flex items-center justify-center text-slate-600 text-sm">{t('dashboard.noTradesInPeriod')}</div>)}
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
        <StatsCard title={t('stats.winRate')} value={formatPct(stats.winRate)} subtitle={`${stats.wins}W / ${stats.losses}L${stats.breakEven > 0 ? ` / ${stats.breakEven}BE` : ''}`} icon={<Target className="w-4 h-4" />} trend={stats.winRate >= 0.5 ? 'up' : 'down'} delay={0} />
        <StatsCard title={t('dashboard.profitFactor')} value={stats.profitFactor >= 99 ? '99+' : stats.profitFactor.toFixed(2)} subtitle={`${t('dashboard.avgRR')} ${stats.avgRR.toFixed(2)}`} icon={<Activity className="w-4 h-4" />} trend={stats.profitFactor >= 1.5 ? 'up' : stats.profitFactor < 1 ? 'down' : 'neutral'} delay={60} />
        <StatsCard title={t('quant.expectancy')} value={formatPnl(quant.expectancy)} subtitle={`${quant.expectancyR >= 0 ? '+' : ''}${quant.expectancyR.toFixed(2)}R / trade`} icon={stats.totalPnl >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />} trend={quant.expectancy >= 0 ? 'up' : 'down'} delay={120} />
        <StatsCard title={t('dashboard.maxDrawdown')} value={formatPnl(-stats.maxDrawdown)} subtitle={quant.maxDrawdownPct !== null ? `${(quant.maxDrawdownPct * 100).toFixed(1)}% · ${t('dashboard.peakToTrough')}` : t('dashboard.peakToTrough')} icon={<BarChart3 className="w-4 h-4" />} trend="down" delay={180} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="col-span-1 md:col-span-2 glass rounded-2xl overflow-hidden card-premium animate-fade-in-up stagger-4">
          ...
          <div className="divide-y divide-white/[0.04]">
            {recentTrades.length === 0 ? (
              <div className="px-4 py-10 text-center text-slate-600 text-sm">{t('dashboard.noTradesInPeriod')}</div>
            ) : recentTrades.map(trade => {
              const be = isBreakEven(trade);
              return (
              <div key={trade.id} className="px-4 md:px-5 py-3 trade-card flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden md:inline text-[10px] text-slate-600">{trade.strategy}</span>
                  </div>
        ...
        <div className="glass rounded-2xl p-4 md:p-5 card-premium animate-fade-in-up stagger-5 space-y-2 md:space-y-3 self-start">
          ...
          {[
            ...
            { label: t('quant.planAdherence'), value: formatPct(quant.planAdherence), color: quant.planAdherence >= 0.8 ? 'text-emerald-400' : 'text-amber-400', dot: quant.planAdherence >= 0.8 ? 'bg-emerald-400' : 'bg-amber-400' },
          ].map(item => (
