...
function Logo({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <a href="#" className="flex items-center gap-2.5 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 rounded-sm">
      ...
      />
      <span className={`font-display font-extrabold tracking-[-0.04em] text-white leading-none ${compact ? "text-[1.15rem]" : "text-[1.3rem]"}`}>
        Trade<span className="logo-text">Vault</span>
      </span>
    </a>
  );
}
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 px-4 py-6 backdrop-blur-md overflow-y-auto" onMouseDown={(e) => e.currentTarget === e.target && onClose()}>
      <div className="modal-in relative my-auto w-full max-w-[400px] overflow-hidden rounded-2xl border border-white/[.08] bg-[#0c1421] shadow-[0_30px_90px_rgba(0,0,0,.6)]">
        ...
        <div className="p-7 sm:p-8">
          ...
          {/* Google — fastest path */}
          <button onClick={google} disabled={loading} className="mt-6 flex w-full items-center justify-center gap-3 rounded-lg border border-white/[.1] bg-white/[.04] py-2.5 text-sm font-semibold text-slate-100 transition hover:border-white/20 hover:bg-white/[.07] disabled:opacity-60">
...
export default function Landing() {
  ...
  // The app's global `body { overflow-x: hidden }` promotes body to a scroll
  // container sized to its content, which swallows wheel/touch events so the
  // page only moves via programmatic scrollIntoView (the navbar). `clip` hides
  // horizontal overflow WITHOUT creating a scroll container, so <html> stays the
  // sole scroller and the wheel works. Scoped to while the landing is mounted.
  useEffect(() => {
  ...
  return (
    <div className="landing-root min-h-screen overflow-x-clip bg-[#060d16] text-slate-100 selection:bg-cyan-400 selection:text-slate-950">
      ...
      {/* ── NAV ── */}
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${y > 10 ? "border-b border-white/[.07] bg-[#060d16]/88 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,.28)]" : "border-b border-transparent"}`}>
