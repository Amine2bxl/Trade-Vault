...
export function computeQuantStats(trades: Trade[], startingBalance = 0): QuantStats {
  const empty: QuantStats = {
    expectancy: 0, expectancyR: 0, sharpe: null, sortino: null,
    maxDrawdownPct: null, kelly: null, bestDayShare: null,
    consistencyScore: null, recoveryDays: null, planAdherence: 0,
  };
  ...
  const expectancyR = withRisk.length > 0
  ...
  let equity = 0, peak = 0, maxDD = 0, peakAtMaxDD = 0;
  let troughDate = '';
  for (const t of sorted) {
    equity += t.pnl;
    if (equity > peak) peak = equity;
    const dd = peak - equity;
    if (dd > maxDD) { maxDD = dd; peakAtMaxDD = peak; troughDate = t.date; }
  }
  ...
  if (maxDD > 0 && troughDate) {
    ...
    let eq = 0, recoveredOn: string | null = null;
    for (const t of sorted) {
      eq += t.pnl;
      if (t.date > troughDate && eq >= peakAtMaxDD) { recoveredOn = t.date; break; }
    }
    recoveryDays = recoveredOn
      ? Math.round((new Date(recoveredOn + 'T12:00:00').getTime() - new Date(troughDate + 'T12:00:00').getTime()) / 86400000)
      : -1;
  }
  ...
  return {
    expectancy, expectancyR, sharpe, sortino, maxDrawdownPct, kelly,
    bestDayShare, consistencyScore, recoveryDays, planAdherence,
  };
}

// ── Breakdown helpers (heatmaps / per-bucket win rates) ─────────────────────
export interface BucketStat { pnl: number; count: number; wins: number; breakEven: number; }
