...
import { X, Target, Quote, Activity, Lightbulb, Compass, Image as ImageIcon, TrendingUp } from 'lucide-react';
...
function Section({ tone, icon: Icon, label, children, big, empty }: {
    ...
    <div className={cn(
      ...
      <div className={cn('py-3.5 px-4 md:px-5', big && !empty && 'py-4 md:py-5')}>
        <div className="flex items-center gap-2 mb-2">
          <div className={cn('rounded-lg flex items-center justify-center shrink-0', s.iconBg, big ? 'w-7 h-7' : 'w-6 h-6')}>
            <Icon className={cn(s.text, big ? 'w-4 h-4' : 'w-3.5 h-3.5')} />
          </div>
          <span className={cn('uppercase tracking-[0.12em] font-bold', s.text, big ? 'text-[11px]' : 'text-[10px]')}>{label}</span>
        </div>
        {children}
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      <div className="relative glass-strong rounded-t-3xl md:rounded-3xl max-w-2xl w-full max-h-[94vh] md:max-h-[88vh] overflow-hidden animate-slide-up md:animate-slide-in shadow-2xl shadow-black/50 flex flex-col">
        ...
        <div className="relative px-4 md:px-7 pt-3 md:pt-6 pb-3.5 md:pb-5 border-b border-white/[0.06] bg-gradient-to-b from-amber-500/[0.06] to-transparent overflow-hidden shrink-0">
          ...
          <div className="relative flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button onClick={onClose} className="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-3.5 md:p-7 space-y-3 md:space-y-4 pb-[calc(env(safe-area-inset-bottom,0px)+16px)] md:pb-7">
          <Section tone="red" icon={Quote} label={t('missed.card.why')} empty={!missed.reasonNotTaken}>
            {body('reasonNotTaken')}
          </Section>

          <Section tone="amber" icon={Activity} label={t('missed.card.what')} empty={!missed.whatHappened}>
            {body('whatHappened')}
          </Section>

          {/* Lesson learned — key takeaway, strongest visual weight */}
          <Section tone="blue" icon={Lightbulb} label={t('missed.card.lesson')} big empty={!missed.lessonLearned}>
          ...
          <Section tone="emerald" icon={Compass} label={t('missed.card.next')} empty={!missed.nextTimePlan}>
            {body('nextTimePlan')}
          </Section>
          ...
          {shots.length > 0 && (
            <div className="pt-1">
              <div className="flex items-center gap-2 mb-2.5 md:mb-3">
                ...
                <span className="text-[10px] uppercase tracking-[0.12em] text-slate-500 font-semibold">{t('tradeDetail.chartScreenshots')}</span>
