...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ThemeContextValue>(() => ({
    ...
    updateTheme: (id, patch) =>
      setStore((s) => ({ ...s, custom: s.custom.map((t) => (t.id === id ? { ...t, ...patch } : t)) })),
    duplicateTheme: (id) => {
      const src = findTheme(store, id);
      const nid = rid();
      const copy: ThemeDef = { id: nid, name: `${src.name} copy`, primary: src.primary, secondary: src.secondary, highlight: src.highlight };
