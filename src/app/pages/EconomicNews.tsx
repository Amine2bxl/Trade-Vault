...
const IMPACT_LABELS: Record<string, string> = { high: "Fort", medium: "Moyen", low: "Faible", holiday: "Férié" };
...
export default function EconomicNews() {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      if (dayPreset === "all" && customDayFilter && localDayIso(e) !== customDayFilter) return false;
  ...
  }, [events, currencyFilter, impactFilter, dayFilter, dayPreset, customDayFilter, search, localDayIso]);
  ...
  const activeFilterCount =
    currencyFilter.size + impactFilter.size + (search.trim() ? 1 : 0);
    ...
    <div className="p-4 md:p-5 max-w-[1100px] mx-auto">
      <PageHeader
        ...
        actions={
          <span className={cn(
            ...
            liveActive ? "bg-emerald-500/10 border-emerald-500/25 text-emerald-300" : "bg-white/[0.04] border-white/[0.08] text-slate-400",
          )}>
            {liveActive && <span className="relative flex w-1.5 h-1.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-60" /><span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-400" /></span>}
            {t("news.live")}
          </span>
        }
      />
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="flex items-center gap-1 bg-white/[0.03] border border-white/[0.07] rounded-xl p-1 shrink-0">
          <button onClick={() => { setWeekStart((w) => addDays(w, -7)); }} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-white transition shrink-0">
          ...
          {!isThisWeek && (
            <button onClick={jumpToToday} className="h-8 px-2.5 rounded-lg flex items-center gap-1 text-[10px] font-bold text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 transition shrink-0">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("news.searchPlaceholder")} className="w-full h-10 bg-white/[0.03] border border-white/[0.07] rounded-xl pl-9 pr-8 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-cyan-500/40" />
          {search && <button onClick={() => setSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg flex items-center justify-center text-slate-500 hover:text-white"><X className="w-3.5 h-3.5" /></button>}
        </div>
        <button onClick={() => setFiltersOpen((v) => !v)} className={cn("relative h-10 px-3 rounded-xl flex items-center gap-1.5 text-xs font-semibold border transition shrink-0", filtersOpen || activeFilterCount > 0 ? "bg-cyan-500/15 border-cyan-500/30 text-cyan-300" : "bg-white/[0.03] border-white/[0.07] text-slate-300 hover:bg-white/[0.06]")}>
          <SlidersHorizontal className="w-3.5 h-3.5" />
          {activeFilterCount > 0 && <span className="min-w-[18px] h-[18px] px-1 rounded-full bg-cyan-500 text-[10px] font-bold text-white flex items-center justify-center">{activeFilterCount}</span>}
        </button>
      </div>
      ...
      {!loading && events.length > 0 && (
        <div className="flex items-center gap-1 mb-4 overflow-x-auto pb-1">
          {([
            { preset: "today" as DayNavPreset, label: "Aujourd'hui", icon: <Clock className="w-3 h-3" /> },
            { preset: "tomorrow" as DayNavPreset, label: "Demain", icon: <ChevronRight className="w-3 h-3" /> },
            { preset: "week" as DayNavPreset, label: "Cette semaine", icon: <CalendarDays className="w-3 h-3" /> },
            { preset: "all" as DayNavPreset, label: "Tout", icon: null },
          ]).map(({ preset, label, icon }) => {
            const active = dayPreset === preset;
            const count = preset === "today"
              ? days.find(d => d.iso === todayIso)?.all.length ?? 0
              : preset === "tomorrow"
                ? days.find(d => d.iso === tomorrowIso)?.all.length ?? 0
                : preset === "week" ? events.length : 0;
            return (
              <button key={preset} onClick={() => { setDayPreset(preset); if (preset === "today") setCustomDayFilter(null); }} className={cn(
                "shrink-0 flex items-center gap-1.5 h-9 px-3 rounded-xl border text-xs font-semibold transition",
                active ? "bg-cyan-500/15 border-cyan-500/30 text-cyan-200" : "bg-white/[0.02] border-white/[0.06] text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]",
              )}>
                {icon}
                {label}
                {preset !== "all" && <span className={cn("text-[10px] font-bold tabular-nums", active ? "text-cyan-300/70" : "text-slate-600")}>{count}</span>}
              </button>
            );
          })}
          ...
          {IMPACTS.map((i) => {
            ...
            return (
              <button key={i} onClick={() => toggle(setImpactFilter, i)} className={cn(
                ...
                on ? cn(st.bg, st.ring, st.text) : "bg-white/[0.02] border-white/[0.06] text-slate-500 hover:text-slate-300",
              )}>
      ...
      {dayPreset === "all" && (
        <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
          {days.map(({ iso, date, all }) => {
            ...
            return (
              <button key={iso} onClick={() => setCustomDayFilter(on ? null : iso)} disabled={all.length === 0} className={cn(
                "shrink-0 h-10 px-3 rounded-xl border text-xs font-bold transition flex items-center gap-1.5",
                on ? "bg-cyan-500/15 border-cyan-500/30 text-cyan-200" : all.length === 0 ? "bg-white/[0.01] border-white/[0.04] text-slate-700 cursor-not-allowed" : "bg-white/[0.02] border-white/[0.06] text-slate-300 hover:bg-white/[0.05]",
      ...
      {filtersOpen && (
        <Card className="p-3 mb-4 flex flex-wrap items-center gap-2">
          ...
          {activeFilterCount > 0 && (
            <button onClick={clearFilters} className="ml-auto text-[11px] font-semibold text-slate-400 hover:text-white transition">Effacer</button>
          )}
        </Card>
      )}
      ...
      {isThisWeek && nextEvent && (
        <div className={cn(
          ...
          nextEvent.impact === "high" ? "border-red-500/15 bg-red-500/[0.03]"
          : nextEvent.impact === "medium" ? "border-orange-500/15 bg-orange-500/[0.02]"
          : "border-cyan-500/15 bg-cyan-500/[0.02]",
        )}>
          <span className={cn("w-1 self-stretch rounded-full shrink-0", IMPACT_STYLE[nextEvent.impact].bar)} />
          <div className="flex-1 min-w-0">
            <div className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Prochain event</div>
            <div className="text-sm font-semibold text-white truncate mt-0.5">{flagOf(nextEvent.currency)} {nextEvent.title}</div>
            <div className="text-[11px] text-slate-500 mt-0.5">{timeFmt.format(new Date(nextEvent.startsAt))}</div>
          </div>
      ...
      {/* Timeline */}
      {loading ? (
        <div className="space-y-3">{[0, 1, 2].map((i) => (<div key={i} className="glass rounded-2xl h-20 animate-pulse" />))}</div>
      ) : filtered.length === 0 ? (
            ...
            return (
              <div key={iso} ref={isToday ? todayRef : undefined}>
                <div className="flex items-center gap-2 mb-2 px-1">
                  <span className={cn("text-xs font-bold capitalize", isToday ? "text-cyan-300" : "text-slate-400")}>{dayFmt.format(date)}</span>
                  {isToday && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-cyan-500/15 text-cyan-300 border border-cyan-500/25">Aujourd'hui</span>}
                  {highCount > 0 && <span className="flex items-center gap-1 text-[11px] font-semibold text-red-400"><span className="w-1.5 h-1.5 rounded-full bg-red-500" />{highCount}</span>}
                ...
                <Card className="overflow-hidden border-white/[0.04]">
                  {dayEvents.map((e, index) => {
                    ...
                    return (
                      <div key={e.id} className={cn(index > 0 && "border-t border-white/[0.03]", status === "past" && "opacity-35", status === "live" && "bg-emerald-500/[0.04]")}>
                        <div className="relative flex items-stretch">
                          <span className={cn("w-1 shrink-0 rounded-r-sm", s.bar, open ? "opacity-100" : "opacity-25")} />
                          <button onClick={() => setExpanded(open ? null : e.id)} className="flex-1 flex items-center gap-3 pl-3 pr-3 py-2.5 text-left hover:bg-white/[0.02] transition-colors min-w-0">
                            <span className="w-12 shrink-0 text-right font-mono text-xs font-semibold tabular-nums text-slate-400">{e.allDay ? "—" : timeFmt.format(start)}</span>
                            <span className={cn("flex-1 min-w-0 text-sm font-medium truncate", status === "past" ? "text-slate-600" : "text-white")}>{e.title}</span>
                            <span className={cn("shrink-0 h-5 px-1.5 rounded text-[10px] font-bold border flex items-center gap-1", s.bg, s.ring, s.text)}>{flagOf(e.currency)} {e.currency}</span>
                            {status === "live" && <span className="shrink-0 h-5 px-1.5 rounded text-[10px] font-bold uppercase bg-emerald-500/15 text-emerald-300 border border-emerald-500/25 animate-pulse">Live</span>}
                            <ChevronDown className={cn("w-3.5 h-3.5 text-slate-600 shrink-0 transition-transform duration-200", open && "rotate-180")} />
                          </button>
                        </div>
                        {open && (
                          <div className="border-t border-white/[0.04] pl-[72px] pr-4 py-3">
                            <div className="flex items-start gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2">
                              {e.previous && <div className="min-w-0 flex-1"><div className="text-[10px] uppercase text-slate-600 font-bold">Précédent</div><div className="text-sm font-semibold text-slate-300 tabular-nums">{e.previous}</div></div>}
                              {e.forecast && <div className="min-w-0 flex-1"><div className="text-[10px] uppercase text-slate-600 font-bold">Prévision</div><div className="text-sm font-semibold text-slate-300 tabular-nums">{e.forecast}</div></div>}
                            </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-1 px-1 text-[11px] text-slate-600">
        <span>Fuseau : {timeZone}</span><span>·</span>
