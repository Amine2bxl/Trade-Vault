...
export default function Analytics({ trades }: AnalyticsProps) {
      ...
      Object.entries(stats.pnlByDayOfWeek)
        ...
        .map(([day, data]) => {
          ...
          return {
            ...
            trades: data.count,
            winRate: dec > 0 ? Math.round((data.wins / dec) * 100) : 0,
          };
        }),
    [stats.pnlByDayOfWeek, DAY_NAMES],
  );
  const strategyData = useMemo(
    () =>
      Object.entries(stats.pnlByStrategy)
        .map(([strategy, data]) => {
          const dec = data.count - data.breakEven;
          return {
            strategy,
  ...
  const pnlDistribution = useMemo(() => {
    ...
    for (const trade of cutoffTrades) {
      ...
      else if (trade.pnl < 0) b[2].count++;
      else if (trade.pnl < 200) b[4].count++;
      else if (trade.pnl < 500) b[5].count++;
      else b[6].count++;
    }
    return b;
  }, [cutoffTrades, t]);
  ...
  const monthlyData = useMemo(() => {
    ...
    for (const t of cutoffTrades) {
      ...
      map[k].pnl += t.pnl;
      map[k].count++;
      map[k].totalRR += Math.abs(t.rMultiple);
      if (t.direction === "be") map[k].be++;
      else if (t.pnl > 0) map[k].wins++;
    }
    return Object.entries(map)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([key, data]) => {
        const [y, m] = key.split("-");
        const dec = data.count - data.be;
        return {
          month: `${MONTH_NAMES_SHORT[parseInt(m) - 1]} '${y.slice(-2)}`,
          pnl: Math.round(data.pnl * 100) / 100,
          trades: data.count,
          winRate: dec > 0 ? Math.round((data.wins / dec) * 100) : 0,
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 animate-fade-in-up stagger-1">
          ...
          {[
            ...
            },
            {
              ...
              sub: `${quant.expectancyR >= 0 ? "+" : ""}${quant.expectancyR.toFixed(2)}R`,
              good: quant.expectancy >= 0,
              info: t("quant.infoExpectancy"),
            },
            {
              label: "Sharpe",
              value: quant.sharpe !== null ? quant.sharpe.toFixed(2) : "—",
              sub: quant.sharpe === null ? t("quant.needTenDays") : t("quant.annualized"),
              good: (quant.sharpe ?? 0) >= 1,
              info: t("quant.infoSharpe"),
            },
            {
              label: "Sortino",
              value:
                quant.sortino !== null
                  ? quant.sortino >= 99
                    ? "99+"
                    : quant.sortino.toFixed(2)
                  : "—",
              sub: quant.sortino === null ? t("quant.needTenDays") : t("quant.annualized"),
              good: (quant.sortino ?? 0) >= 1.5,
              info: t("quant.infoSortino"),
            },
            {
              label: "Kelly",
              value: quant.kelly !== null ? `${(quant.kelly * 100).toFixed(1)}%` : "—",
              sub: t("quant.kellyHint"),
              good: (quant.kelly ?? 0) > 0,
              info: t("quant.infoKelly"),
            },
            {
              label: t("quant.consistency"),
              value:
                quant.consistencyScore !== null ? `${quant.consistencyScore.toFixed(0)}/100` : "—",
              sub:
                quant.bestDayShare !== null
                  ? `${t("quant.bestDay")} ${(quant.bestDayShare * 100).toFixed(0)}%`
                  : "—",
              good: (quant.consistencyScore ?? 0) >= 60,
              info: t("quant.infoConsistency"),
            },
            {
              label: t("quant.recovery"),
              value:
                quant.recoveryDays === null
                  ? "—"
                  : quant.recoveryDays === -1
                    ? t("quant.inDrawdown")
                    : `${quant.recoveryDays}${t("quant.daysShort")}`,
              sub: t("quant.recoverySub"),
                  ...
                  <tr key={row.strategy} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-xs text-right tabular-nums">
                      <span
                        className={cn(
                          "font-semibold",
                          row.expectancy >= 0 ? "text-emerald-400" : "text-red-400",
                        )}
                      >
              ...
              {SESSIONS.map((s) => (
                <Fragment key={s}>
                  ...
                  {[1, 2, 3, 4, 5].map((d) => {
                    ...
                    const intensity =
                      cell && heatmap.maxAbs > 0 ? Math.abs(cell.pnl) / heatmap.maxAbs : 0;
                    return (
                      <div
                        key={`${s}-${d}`}
            ...
            <div className="flex flex-col gap-0.5 w-full">
              ...
              {years.map(([year, months]) => (
                <div key={year} className="flex gap-0.5 items-center">
                  <div className="w-8 text-[10px] font-bold text-slate-400 shrink-0 text-right pr-1">
                    {year}
                  </div>
                  <div className="grid grid-cols-12 gap-0.5 flex-1">
                    {months.map((val, i) => {
                      ...
                      return (
                        ...
                        >
                          {val === 0 && months.every((v) => v === 0)
                            ? "—"
                            : `${isWin ? "+" : ""}${Math.round(val)}`}
                        </div>
                      );
                    })}
                  </div>
