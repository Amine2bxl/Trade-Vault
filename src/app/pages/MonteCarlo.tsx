...
import { Play, Loader2, BarChart3, Zap, TrendingUp, AlertTriangle, Info, ChevronDown, X } from "lucide-react";
...
import {
  ResponsiveContainer, AreaChart, Area, Line, CartesianGrid, XAxis, YAxis, Tooltip, ReferenceLine, ComposedChart,
} from "recharts";
...
/* ─────── PARAMETER INPUT ─────── */
function ParamRow({ label, value, onChange, suffix = "", computed, hint }: {
  label: string; value: number; onChange: (v: number) => void; suffix?: string; computed?: boolean; hint?: string;
}) {
  return (
    <div className="flex items-center justify-between group">
      <div className="flex items-center gap-1 min-w-0">
        <span className="text-[11px] text-slate-400 truncate">{label}</span>
        {computed && <span className="w-1.5 h-1.5 rounded-full bg-cyan-500/60 shrink-0" title="Auto-computed from your data" />}
...
/* ─────── KPI CARD ─────── */
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color: "emerald" | "red" | "amber" | "cyan" | "white" }) {
  const cls = color === "emerald" ? "text-emerald-400" : color === "red" ? "text-red-400" : color === "amber" ? "text-amber-400" : color === "cyan" ? "text-cyan-400" : "text-white";
  return (
...
/* ─────── EQUITY CURVE CHART ─────── */
function EquityCurves({ result, challenge }: { result: MonteCarloResult; challenge: PropChallenge | undefined }) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const vals = sampled.map((r) => r.equity[Math.min(idx, r.equity.length - 1)]).sort((a, b) => a - b);
      data.push({
        step: idx,
  ...
  return (
    <div className="glass rounded-2xl p-4">
      ...
      <div className="h-64 md:h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 20, right: 10, bottom: 20, left: 0 }}>
            ...
            <YAxis tick={AXIS_TICK} tickFormatter={(v) => formatMoney(v)} axisLine={false} tickLine={false} width={55} domain={["dataMin - 500", "dataMax + 500"]} />
            ...
            {/* P25-P75 band */}
            <Area type="monotone" dataKey="p25" stroke="none" fill="rgba(34,211,238,0.04)" fillOpacity={1} />
            <Area type="monotone" dataKey="p75" stroke="none" fill="rgba(34,211,238,0.04)" fillOpacity={1} />
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-xl border border-white/[0.08] px-3 py-2" style={{ background: "rgba(10,15,30,0.97)" }}>
                    <p className="text-[10px] text-slate-500 font-bold uppercase mb-0.5">Equity Distribution</p>
      ...
      <div className="flex items-center gap-4 mt-2 text-[9px] text-slate-600">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-cyan-400" />Median</span>
        <span className="flex items-center gap-1"><span className="w-2 h-0.5 w-3 bg-cyan-400/20" />P25-P75</span>
        <span className="flex items-center gap-1"><span className="w-2 h-0.5 w-3 bg-cyan-400/10" />P5-P95</span>
      </div>
    </div>
  );
}
...
export default function MonteCarloPage({ trades }: Props) {
  ...
  const [riskPerTrade, setRiskPerTrade] = useState(() => Math.round((computedStats.avgLossPnl || 100)));
    ...
    return (
      <div className="p-4 md:p-5 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-sm">Ajoute au moins 5 trades dans ton journal pour lancer ta première simulation.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-5 max-w-[1400px] mx-auto">
      <PageHeader
        ...
        actions={
          <button onClick={handleRun} disabled={running || trades.length < 5}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 mt-4">
        ...
        <div className="space-y-3">
          {/* Preset */}
          <div className="glass rounded-2xl p-4">
            <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2">Quick Preset</h4>
            <select value={firmId} onChange={(e) => selectFirm(e.target.value)}
              ...
              {[{ id: "apex", name: "Apex Trader Funding" }, { id: "topstep", name: "Topstep" }, { id: "custom", name: "Custom" }].map((f) => (<option key={f.id} value={f.id}>{f.name}</option>))}
            </select>
            <select value={challengeId} onChange={(e) => selectChallenge(e.target.value)}
              ...
              {firm?.challenges.map((c) => (<option key={c.id} value={c.id}>{c.name}</option>))}
            </select>
            <p className="text-[10px] text-slate-500 mt-2 italic">Selecting a preset fills the parameters below — you can override any value.</p>
          </div>
          ...
          <div className="glass rounded-2xl p-4">
            ...
            <div className="space-y-2">
              <ParamRow label="Starting Balance" value={startingBalance} onChange={setStartingBalance} />
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-4">
            ...
            <div className="space-y-2">
              ...
              <ParamRow label="Daily Loss Limit ($)" value={maxDailyLoss} onChange={setMaxDailyLoss} />
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-slate-400">Trailing DD</span>
                <button onClick={() => setTrailingDD(!trailingDD)}
                  className={cn("px-3 py-1 rounded-lg text-[10px] font-bold transition border",
                    trailingDD ? "bg-cyan-500/15 text-cyan-300 border-cyan-500/25" : "bg-white/[0.04] text-slate-500 border-white/[0.08]")}>
                  {trailingDD ? "Trailing" : "Static"}
                </button>
              </div>
              <ParamRow label="Max Trading Days" value={maxTradingDays} onChange={setMaxTradingDays} />
              <ParamRow label="Max Trades / Day" value={maxTradesPerDay} onChange={setMaxTradesPerDay} />
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-4">
            ...
            <div className="space-y-2">
              <ParamRow label="Risk per Trade ($)" value={riskPerTrade} onChange={setRiskPerTrade} />
              {startingBalance > 0 && (
                <div className="flex items-center justify-between text-[10px]">
                  <span className="text-slate-500">Risk %</span>
                  <span className="text-slate-400 tabular-nums font-semibold">{((riskPerTrade / startingBalance) * 100).toFixed(1)}%</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="flex gap-1">
              {SIM_OPTIONS.map((n) => (
                <button key={n} onClick={() => setSimCount(n)}
                  className={cn("flex-1 h-8 rounded-lg text-[10px] font-bold transition",
                    simCount === n ? "bg-cyan-500/15 text-cyan-300 border border-cyan-500/25" : "bg-white/[0.03] text-slate-500 border border-white/[0.06] hover:text-slate-300")}>
                  {n >= 1000 ? `${n / 1000}k` : n}
                </button>
              ))}
            </div>
          ...
          {result ? (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <KpiCard label="Pass Rate" value={`${(result.passRate * 100).toFixed(1)}%`} sub={`±${(se * 100).toFixed(1)}% SE`} color="emerald" />
                ...
                <KpiCard label="Avg P&L (Pass)" value={formatPnl(result.avgFinalBalancePassing - result.params.startingBalance)} color="emerald" />
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <KpiCard label="Median Max DD" value={formatMoney(result.medianMaxDD)} color="red" />
                <KpiCard label="P95 Final Balance" value={formatMoney(result.finalBalanceDistribution.p95)} color="white" />
              </div>

              {/* Confidence intervals */}
              <div className="glass rounded-2xl p-4">
                <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2">Final Balance Distribution</h4>
                  ...
                  {(["p5","p25","p50","p75","p95"] as const).map((k) => {
                    ...
                    return (
                      <div key={k} className="flex-1 flex flex-col items-center gap-1">
                        <span className="text-[9px] text-slate-500 tabular-nums">{formatMoney(v)}</span>
                        <div
                          className={cn("w-full rounded-t", k === "p50" ? "bg-cyan-500/60" : "bg-white/[0.08]")}
                          style={{ height: `${Math.max(2, h)}%` }}
                        />
                        <span className="text-[8px] text-slate-600 uppercase">{k.toUpperCase()}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="glass rounded-2xl p-4 flex items-start gap-2.5">
                ...
                <div className="text-[10px] text-slate-500 leading-relaxed">
                  ...
                  <p>Bootstrap resampling from your {computedStats.totalSamples} trades with replacement. Each simulation draws independently from your actual R-multiple distribution, preserving the correlation between win/loss direction and magnitude. This is a Monte Carlo estimate — results reflect historical patterns, not predictions.</p>
                </div>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-slate-500">Configure your parameters and run a simulation</p>
              {trades.length < 5 && <p className="text-[11px] text-amber-400/80 mt-2">Minimum 5 trades required</p>}
            </div>
          )}
        </div>
      </div>
...
function StatRow({ label, value, computed, accent }: { label: string; value: string; computed?: boolean; accent?: string }) {
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-1">
        {computed && <span className="w-1.5 h-1.5 rounded-full bg-cyan-500/60 shrink-0" title="Computed from your data" />}
        <span className="text-[10px] text-slate-500">{label}</span>
      </div>
      <span className={cn("text-[10px] font-semibold tabular-nums", accent || "text-white")}>{value}</span>
    </div>
  );
}
