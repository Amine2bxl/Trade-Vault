import type { PointerEvent as RPointerEvent, Dispatch, SetStateAction } from "react";
...
/**
 * Les sections de la landing situées SOUS la ligne de flottaison.
 *
 * POURQUOI UN MODULE À PART. La route `/` importe `Landing.tsx` en statique —
 * à raison : c'est sa surface SSR et son référencement. Mais un visiteur juge
 * la page sur le héros ; le problème, le coach IA, les fonctionnalités, les
 * tarifs et la FAQ n'ont aucune raison d'être téléchargés avant que le héros
 * soit peint.
 *
 * Chargé par `import()` dès que le navigateur est libre — pas au défilement :
 * quelqu'un qui fait défiler immédiatement ne doit jamais attendre un
 * aller-retour réseau.
 *
 * L'état reste dans `Landing`. Ce composant ne possède rien, il reçoit.
 */

interface BelowFoldProps {
  /** Défilement animé vers une section, avec verrouillage du scrollspy. */
  go: (id: string) => void;
  /** Ouvre la modale d'authentification (connexion ou inscription). */
  open: (mode: "login" | "signup", plan?: string) => void;
  /** Effet de projecteur suivant le pointeur sur les cartes. */
  spot: (e: RPointerEvent<HTMLElement>) => void;
  /** Index de la question ouverte dans la FAQ. */
  faq: number | null;
  setFaq: Dispatch<SetStateAction<number | null>>;
  /** Compte à rebours jusqu'à l'ouverture des marchés, ou null. */
  cd: string | null;
}
...
export default function BelowFold({ go, open, spot, faq, setFaq, cd }: BelowFoldProps) {
  return (
    <>
      {/* ── PROBLÈME ── */}
      ...
      >
        ...
        <div className="relative mx-auto max-w-[1200px] px-5 lg:px-8">
          <SectionHead
            ...
            title={
              <>
                Un coach IA qui connaît <span className="text-gradient">chacun de tes trades.</span>
              </>
            }
        ...
        <div className="mx-auto max-w-[1200px] px-5 lg:px-8">
          ...
          <div className="reveal mb-10 flex justify-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-400/[.08] px-4 py-1.5 text-xs font-bold text-emerald-300">
              <Icon n="sparkle" cls="h-3.5 w-3.5" /> En passant à l'année : 2 mois offerts, soit 40
              € d'économie
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3 lg:items-stretch">
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-500">
                Pour <span className="text-slate-300">noter</span> tes trades et poser les bases de
                ta discipline.
              </p>
              <button
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-500">
                Soit{" "}
                <span className="font-semibold text-slate-400">{eur(YEARLY_FULL_PRICE)} / an</span>{" "}
          ...
          <div className="reveal mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
            {[
              ...
              <span key={t} className="flex items-center gap-2 text-xs font-medium text-slate-500">
