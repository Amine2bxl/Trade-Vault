    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_.85fr_.85fr]">
      ...
      <div className="reveal">
        ...
        <div className="mt-8 grid grid-cols-3 gap-4">
          {FACTS.map((f) => (
            <div key={f.v}>
              <p className="font-display text-[clamp(1.5rem,3vw,2rem)] font-bold text-[var(--lp-accent-soft)]">
                {f.v}
              </p>
      ...
      <div className="glass-card reveal flex flex-col justify-between px-6 py-7">
        <Quote className="h-7 w-7 text-[var(--lp-accent)]/40" aria-hidden="true" />
        <p className="mt-4 text-[14px] leading-7 text-slate-300">{t("proof.quote")}</p>
        <div className="mt-6 flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-full border border-[rgb(var(--lp-accent-rgb)/0.25)] bg-[rgb(var(--lp-accent-rgb)/0.10)] font-display text-sm font-bold text-[var(--lp-accent-soft)]">
            TV
          </div>
          <div>
