...
export default function HomeWorkspace({ context }: JarvisWorkspaceProps) {
  ...
  return (
    <div className="mx-auto min-h-0 w-full max-w-[1100px] flex-1 overflow-y-auto px-4 py-4 md:px-8 md:py-6">
      {/* ── L'EN-TÊTE ──────────────────────────────────────────────────────
          Il tenait sur ~180px avant la première information : une marque de
          56px cerclée d'un halo cyan flou, un point vert clignotant surmonté du
          mot « Jarvis », un titre en 30px, une ligne « pose-moi une question »,
          puis la phrase d'accueil. La marque et le nom sont déjà posés dans la
          bande de tête de la fenêtre, quarante pixels plus haut : les répéter
          ici, c'est dire deux fois la même chose et faire descendre d'autant ce
          que Jarvis a réellement à dire.

          Reste ce qui n'existe qu'ici : à qui il parle, et sa phrase du jour —
          celle qui est CALCULÉE depuis les trades, pas une accroche. */}
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="tv-prose mt-0.5 max-w-xl text-slate-400">
            {welcomeLine ?? t("jarvisHome.ask")}
          </p>
        </div>
        {/* La parole est un ÉTAT transitoire : elle mérite un signe, pas une
            pastille encadrée qui pousse le contenu vers le bas. */}
        {speaking && (
          <span
            className="tv-label inline-flex shrink-0 items-center gap-1.5 text-[var(--tv-highlight)]"
            title={t("jarvisHome.speaking")}
          >
            <Volume2 className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">{t("jarvisHome.speaking")}</span>
          </span>
        )}
      </div>
      ...
      {worstMistake && !ruleAdded && (
        /* Une carte qui PORTE UNE ACTION a le droit à sa présence : le liseré
           d'accent la désigne. Ce qui part, c'est le décor — le dégradé sur
           deux teintes et le halo flou derrière la pastille. */
        <div className="mt-5 rounded-2xl border border-[var(--tv-border-accent)] bg-[var(--tv-plate-1)] p-3.5">
          <div className="flex items-start gap-3">
            <span className="tv-accent-fill grid h-8 w-8 shrink-0 place-items-center rounded-xl">
              <Sparkles className="h-4 w-4" />
            </span>
            <div className="min-w-0 flex-1">
              <div className="tv-label mb-1 text-[var(--tv-highlight)]">
      ...
      {ruleAdded && (
        /* Un accusé de réception n'a plus aucune interaction : c'est une ligne,
           pas une carte. */
