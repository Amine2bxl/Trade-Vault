  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      >
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <p className="text-sm text-slate-400">
                  {trades.length}{" "}
                  {trades.length !== 1
                    ? t("tradeDetail.tradesCountSuffix")
                    : t("tradeDetail.tradeCountSuffix")}
                </p>
                {macro.map((ev) => (
                  <span
                    key={ev}
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <button
                ...
                className="w-11 h-11 md:w-9 md:h-9 rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/5 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="overflow-y-auto max-h-[calc(96vh-100px)] md:max-h-[calc(88vh-90px)] p-4 md:p-5 pb-[calc(env(safe-area-inset-bottom,0px)+16px)] md:pb-5 space-y-3 md:space-y-4">
          {missed.length > 0 && (
            ...
            return (
              <div key={trade.id} className="glass rounded-2xl p-5 space-y-4 card-premium">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                  ...
                  <div className="bg-white/[0.03] rounded-xl p-3 text-center">
                    <div className="text-[10px] text-slate-500 mb-1">
                      {t("tradeDetail.duration")}
                    </div>
                    <div className="text-sm font-semibold text-white">
                      {getDuration(trade.entryTime, trade.exitTime)}
                    </div>
                  </div>
                  <div className="bg-white/[0.03] rounded-xl p-3 text-center">
                  ...
                  <div className="bg-white/[0.03] rounded-xl p-3 text-center">
                    ...
                    <div className="text-[10px] text-slate-500 mb-0.5">
                      {t("tradeDetail.quality")}
                    </div>
                    <div className="text-sm font-semibold text-slate-300">
                      {"★".repeat(trade.setupQuality)}
                      {"☆".repeat(5 - trade.setupQuality)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 md:gap-4 text-xs">
                  ...
                  {trade.riskAmount > 0 && (
                    <div className="bg-white/[0.03] rounded-lg px-3 py-2">
                      <span className="text-slate-500">{t("tradeDetail.pnlPerRisk")}: </span>
                      <span
                        className={cn(
                          "font-semibold",
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {trade.confluences.map((c) => (
                        <span
                          key={c}
                          className="px-3 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/15 text-cyan-400 text-xs font-medium"
                        >
                          {c}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {trade.mistakes.map((m) => (
                        <span
                          key={m}
                          className="px-2 py-1 rounded-lg bg-red-500/10 text-red-400 text-[10px] font-medium"
                        >
                          {m}
                        </span>
                      ))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {trade.screenshots.map((shot, i) => (
                        <button
                          ...
                          onClick={() =>
                            setLightbox({
                              images: trade.screenshots.map((s) => screenshotUrls[s] || ""),
                              index: i,
                            })
                          }
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                            <span className="opacity-0 group-hover:opacity-100 transition-opacity text-xs text-white font-medium bg-black/40 px-2 py-1 rounded-md">
                              {t("common.view")}
                            </span>
                          </div>
                        </button>
