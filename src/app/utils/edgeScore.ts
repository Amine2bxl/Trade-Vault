...
export function computeEdgeScore(allTrades: Trade[], inputs: EdgeInputs = {}): EdgeResult {
  ...
  subs.plan = { value: (window.filter((t) => t.mistakes.length === 0).length / window.length) * 100 };
  ...
  if (daysWithChecklist.length > 0) {
    const sum = daysWithChecklist.reduce((acc, d) => acc + Math.max(0, Math.min(1, checklist[d])), 0);
    subs.routine = {
