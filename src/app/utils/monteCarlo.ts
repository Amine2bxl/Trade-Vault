...
export function extractRSamples(
  trades: { pnl: number; rMultiple: number; direction: "long" | "short" | "be" }[],
): RMultipleSample[] {
  return trades.map((t) => ({
    r: t.rMultiple,
    pnl: t.pnl,
...
export function computeStatistics(samples: RMultipleSample[]): SampleStatistics {
  ...
  const variance =
    samples.length > 0 ? samples.reduce((a, s) => a + (s.r - meanR) ** 2, 0) / samples.length : 0;
      ...
      if (dd >= maxDrawdown) {
        ...
        return {
          ...
          failReason: trailingDrawdown
            ? `Trailing drawdown hit (day ${day})`
            : `Max drawdown reached (day ${day})`,
          tradingDays: day,
          totalTrades: sampledR.length,
        };
      }
...
export function runMonteCarlo(
  ...
  return {
    ...
    avgFinalBalancePassing:
      passed.length > 0 ? passed.reduce((a, r) => a + r.finalBalance, 0) / passed.length : 0,
    avgDaysToPass: passed.length > 0 ? passDays.reduce((a, d) => a + d, 0) / passed.length : 0,
    avgTradesToPass:
      passed.length > 0 ? passed.reduce((a, r) => a + r.totalTrades, 0) / passed.length : 0,
