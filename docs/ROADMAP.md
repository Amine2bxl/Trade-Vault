...
- **Monétisation** : infra payante conservée, flag `AI_REQUIRE_PRO=false` (OFF).
  Au lancement, un Free qui *fait goûter* l'IA (quota), jamais un Free sans IA.
...
- *(Reste : Phase 2 pruning deps — bloqué par l'egress registry privé, issue #46 ;
  Phase 5 UI par features/ — au fil des lots produit, issue #49.)*
...
*(La « preuve sociale honnête » n'est plus une action : décision prise de garder
Trustpilot en l'état, vrais avis en cours.)*
...
- *(Le badge Trustpilot 5★ n'est plus à retirer : conservé, vrais avis en cours.)*
...
> Format : *Problème — impact → Solution.* **Priorité**.

**Proposition de valeur**
- Promesse « coach IA » vs réalité « boîte Q&A » — sur-vente → déception à
  l'activation → faire vivre l'IA (mémoire + proactivité). **P0** — *en cours (P0 #1 fait)*.
...
- *(Preuve sociale : Trustpilot conservé, vrais avis en cours — pas d'action.)*
