...
import {
  computeQuantStats, getSession, getMacroEvents,
  statsBySession, statsByHour, dayHourMatrix, winRateOf,
...
describe('computeQuantStats', () => {
  ...
  it('kelly = W - (1-W)/R', () => {
    ...
    const q = computeQuantStats([
      mkTrade({ pnl: 200 }), mkTrade({ pnl: 200 }),
      mkTrade({ pnl: -100 }), mkTrade({ pnl: -100 }),
    ]);
